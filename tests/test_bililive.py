...
class BiliLiveAPITests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_browser_fallback_is_attempted_after_empty_http_sources(self):
        ...
        with patch("bot_tools.bililive._fetch_dynamic_api", new_callable=AsyncMock, return_value=empty), patch(
            "bot_tools.bililive._fetch_dynamic_in_browser", new_callable=AsyncMock, return_value=self.feed()
        ) as browser:
    ...
    async def test_feed_deadline_is_safe_error(self):
        with patch("bot_tools.bililive._fetch_dynamic_feed", new_callable=AsyncMock, side_effect=TimeoutError):
            with self.assertRaises(ToolError):
                await fetch_dynamic(2)

    async def test_fetch_cancellation_propagates(self):
        with patch("bot_tools.bililive._fetch_dynamic_feed", new_callable=AsyncMock, side_effect=asyncio.CancelledError):
            with self.assertRaises(asyncio.CancelledError):
                await fetch_dynamic(2)

    async def test_browser_tries_desktop_after_web_risk_and_closes_page(self):
        from bot_tools.bililive import _fetch_dynamic_in_browser
        page = AsyncMock()
        page.evaluate.side_effect = [{"code": -412}, self.feed()]
        context = AsyncMock()
        context.new_page.return_value = page
        with patch("bot_tools.bililive._browser", new_callable=AsyncMock, return_value=context):
    ...
    async def test_user_lookup_cancellation_propagates(self):
        with patch("bot_tools.bililive._fetch_user", new_callable=AsyncMock, side_effect=asyncio.CancelledError):
            with self.assertRaises(asyncio.CancelledError):
                await fetch_user(2)

    async def test_user_lookup_timeout_is_safe_error(self):
        with patch("bot_tools.bililive._fetch_user", new_callable=AsyncMock, side_effect=TimeoutError):
