...
三个插件完整拆分为独立安装包，业务代码、资源和专属测试在对应仓库：

| 插件仓库 | 包含内容 |
| --- | --- |
| [FF14](https://github.com/lchlzk/ff14-ddl-qbot-plugin-ff14) | 市场/制作/战绩查询、副属性、海钓、狩猎、群狩猎设置、物品索引及生成工具 |
| [嘟嘟脸](https://github.com/lchlzk/ff14-ddl-qbot-plugin-trickcal) | 角色查询、蜡笔板网页/账号/API、节点与资料更新 |
| [B站](https://github.com/lchlzk/ff14-ddl-qbot-plugin-bililive) | 订阅存储、抓取与轮询推送、浏览器备用查询、动态卡片绘制 |

FF14/B站 0.2.0 需要与新版主体一起更新。原数据路径与记录格式保留，升级不清空用户数据。主体只提供数据库、权限、网络、媒体上传、群设置扩展等公共服务；不安装 FF14 时无副属性/海钓/狩猎命令和狩猎设置，不安装 B站时无需其 Playwright 浏览器。标准镜像默认包含三者；按需部署可在构建前从版本锁定文件中移除不需要的插件行，然后重建镜像。不要保留旧安装中的 `plugins/ffxiv_tools.py` 等本地同名文件，否则会遮蔽已安装的独立插件。
